...
//   2. Read-Back gegen v_radar_lead_state:
//        - do_not_contact (Opt-out)  → skip 'opt_out'
//        - existiert schon als Lead  → skip 'already_in_sales'
//   3. Abgleich gegen radar_targets (dedup_key):
//        - promoted/dismissed/duplicate → skip 'target_<status>'
//        - vorhanden (scored/…)         → Enrichment-Update → 'updated'
//        - neu                          → Insert (status='scored') → 'created'
//
// Schreibt NUR in die Staging-Tabelle radar_targets. Die Übernahme in echte
// leads bleibt der menschliche Promote-Schritt (promote_radar_target).
...
function leadFromRow(row: Record<string, unknown>, targetId: string, key: string) {
...
export async function POST(req: NextRequest) {
  ...
  //     (Websuche → Seite lesen → LLM verifiziert) überschreibt OSM-Vermutungen
  ...
  //     fehlenden). Best-effort, bricht den Ingest nie.
  ...
  if (researchEnabled()) {
    ...
    } catch { /* Research darf den Ingest nie brechen */ }
  } else {
    ...
    await Promise.all(
      noSite.map(async (m) => {
        ...
        } catch { /* Discovery darf den Ingest nie brechen */ }
      }),
    )
  }
  ...
  for (const m of mapped) {
    ...
    const { lead, missing } = leadFromRow(row, targetId as string, key)
  ...
  // Nur wenn RADAR_EMIT_CONTRACT=1: für frisch angelegte/aktualisierte und
  // qualifizierte Leads den Lead-Contract bauen und via sendLead() ausgeben
  // (aktuell Stub → lokale Outbox). Fire-and-forget: darf den Ingest NIE brechen
  // und verändert die Response nicht.
  if (process.env.RADAR_EMIT_CONTRACT === '1') {
